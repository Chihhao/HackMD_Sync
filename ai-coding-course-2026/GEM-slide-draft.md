...
// VISUAL
左右分割對比圖。左側是灰階的、線框化的官方 Web UI 界面，標註著紅色的「X」符號；右側是全彩的、帶有「科技青」光暈的 AI 小旺 Coding 模式界面，突出代碼樹狀結構與多模態輸入區。
// LAYOUT
...
// VISUAL
格式轉換流水線圖。左側是 Word/Excel 圖標被劃掉，右側是 Markdown 圖標轉化為精美的文檔預覽。中間用虛線箭頭連接，象徵無縫轉換。
// LAYOUT
