...
* **資安第一 (Security First)**: 
  * **脫敏原則**: 即使是內部工具，API 仍為外部服務。嚴禁上傳 Project Code Name、客戶名稱、具體良率。
  * **核心 IP 保護**: 僅擷取邏輯段落，不整份上傳架構文件。
...
* **資安意識 (Security First)**
  * **脫敏原則**: 雖然工具是內部的，但 **API 是外部的 (如 GPT-5)**，必須將 **Project Code Name**、**具體良率數值** 或 **客戶名稱** 替換為代號 (如 `Project_A`)。
  ...
  * **上下文 (Context) 與記憶**:
    ...
    * **Token 概念**: 
      ...
      * **Context Window (上下文視窗)**:
        * **GPT-5**: 約 **27 萬** Tokens (公司目前統一使用 OpenAI 旗艦模型)。
        * **600KB 限制**: 小旺 Coding 模式限制 600KB 原始碼 $\approx$ 15 萬 ~ 20 萬 Tokens (約 **1.2 萬 ~ 1.5 萬行**程式碼)。
